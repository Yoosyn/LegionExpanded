...
namespace AmigaNet.Amos.Screens
{
    public class ScreensManager : IScreensManager
    {
        ...
        /// <summary>
        /// Bumped whenever a static-terrain zone (id &gt;= 21) on the *current*
        /// screen is added, moved or cleared, so callers (e.g. pathfinding) can
        /// cheaply detect that their cached view of the obstacle layout for that
        /// screen is stale. Per-unit occupancy zones (id 1-20, rewritten every
        /// frame) deliberately do NOT bump this. Scoped per-screen (see
        /// <see cref="Screen.ZoneEpoch"/>) so churn on one screen (e.g. a UI
        /// stats panel) never invalidates another screen's (e.g. the battle
        /// map's) cached grid.
        /// </summary>
        public int ZoneEpoch => GetCurrentScreen().ZoneEpoch;
        ...
        /// <summary>
        /// Enumerates the non-null zones registered on the current screen, for
        /// consumers (e.g. pathfinding) that need to rasterize obstacles instead
        /// of point-testing them one at a time via <see cref="Zone(int, int)"/>.
        /// </summary>
        public IEnumerable<Zone> GetZones()
        {
            var screen = GetCurrentScreen();
            if (screen.Zones == null) yield break;
            foreach (var zone in screen.Zones)
            {
        ...
        /// <summary>
        /// Width/height (in screen coordinates) of the current screen.
        /// </summary>
        ...
        /// <summary>
        /// FADE
        /// instruction: smoothly fade the current screen's palette to black
        /// Fade speed
        /// </summary>
        public void Fade(int speed)
        {
            var screen = GetCurrentScreen();
            var target = new Pixel[screen.Palette.Length];
            for (var i = 0; i < target.Length; i++)
            {
                target[i] = new Pixel(0, 0, 0, i);
            }
            StartFade(screen, target, speed);
        }

        /// <summary>
        /// FADE ... TO ...
        /// instruction: smoothly fade the current screen's palette towards
        /// another screen's stored palette
        /// Fade speed To screenNumber
        /// </summary>
        public void FadeTo(int speed, int screenNumber)
        {
            var screen = GetCurrentScreen();
            var source = GetScreen(screenNumber);
            var target = new Pixel[source.Palette.Length];
            Array.Copy(source.Palette, target, source.Palette.Length);
            StartFade(screen, target, speed);
        }

        /// <summary>
        /// FADE
        /// instruction: smoothly fade the current screen's palette towards an
        /// explicit list of target colours (AMOS "Fade speed,c0,c1,...").
        /// A null entry in <paramref name="targetColors"/> means "leave this
        /// palette index unchanged" (AMOS's blank/omitted argument, e.g.
        /// "Fade speed,,$FFF,...").
        /// </summary>
        public void Fade(int speed, Pixel[] targetColors)
        {
            var screen = GetCurrentScreen();
            var target = new Pixel[screen.Palette.Length];
            for (var i = 0; i < target.Length; i++)
            {
                var wanted = i < targetColors.Length ? targetColors[i] : null;
                target[i] = wanted ?? screen.Palette[i];
            }
            StartFade(screen, target, speed);
        }

        private void StartFade(Screen screen, Pixel[] target, int speed)
        {
            var from = new Pixel[screen.Palette.Length];
            Array.Copy(screen.Palette, from, screen.Palette.Length);
            screen.ActiveFade = new FadeState
            {
                From = from,
                To = target,
        ...
        /// <summary>
        /// Advances any in-progress FADE by one VBL tick. Called from WaitVbl,
        /// matching AMOS where FADE progresses in the background on every VBL
        /// regardless of what else the program is doing.
        /// </summary>
        private void AdvanceFades()
        {
            // ToArray(): WaitVbl (and therefore this) can be called from a
            // background Amal thread (e.g. a looping sprite/Bob animation)
            // concurrently with the main thread adding/removing screens.
            foreach (var screen in screens.ToArray())
            {
                var fade = screen.ActiveFade;
                if (fade == null) continue;

                fade.TicksSinceStep++;
                if (fade.TicksSinceStep < fade.Speed) continue;
                fade.TicksSinceStep = 0;
                fade.CurrentStep++;

                var t = (float)fade.CurrentStep / fade.TotalSteps;
                if (t > 1f) t = 1f;

                var paletteLength = Math.Min(screen.Palette.Length, Math.Min(fade.From.Length, fade.To.Length));
                for (var i = 0; i < paletteLength; i++)
                {
                    var from = fade.From[i];
                    var to = fade.To[i];
                    if (from == null || to == null) continue;
                    var r = (byte)(from.R + (to.R - from.R) * t);
                    var g = (byte)(from.G + (to.G - from.G) * t);
                    var b = (byte)(from.B + (to.B - from.B) * t);
                    screen.Palette[i] = new Pixel(r, g, b, i);
                }

                RebakeScreenPixels(screen, paletteLength);

                if (fade.CurrentStep >= fade.TotalSteps)
                {
                    screen.ActiveFade = null!;
                }
            }
        }

        /// <summary>
        /// Re-applies the (just updated) palette to every already-drawn pixel
        /// on the screen bitmap that was painted using one of the changed
        /// palette indices. This is needed because, unlike real Amiga
        /// hardware (which resolves indexed colour at scan-out time so a
        /// palette change instantly recolours everything on screen), pixels
        /// here have their RGB resolved once at draw time. Only screen.Data
        /// (text/bars/lines and background pictures) is re-baked; Bobs and
        /// sprites draw from a separate shared bank palette and are out of
        /// scope for now.
        /// </summary>
        private void RebakeScreenPixels(Screen screen, int paletteLength)
        {
            ...
            for (var i = 0; i < data.Length; i++)
            {
                ...
                if (index < 0 || index >= paletteLength) continue;
                data[i] = screen.Palette[index];
            }
            screen.IsModified = true;
        }

        /// <summary>
        /// Converts an AMOS packed 12-bit $RGB colour literal (each nibble
        /// 0-15) into a Pixel with byte-range (0-255) components.
        /// </summary>
        public static Pixel AmosHex(int packedRgb)
        {
            var r = (packedRgb >> 8) & 0xF;
            var g = (packedRgb >> 4) & 0xF;
            var b = packedRgb & 0xF;
            return new Pixel((byte)(r * 17), (byte)(g * 17), (byte)(b * 17));
        }
        ...
        public void AmalRun(int channelNumber, Sprite sprite, List<AmalInstruction> instructions)
        {
            var info = new AmalInfo
            {
                Number = channelNumber,
        ...
        private void AmalThreadRun(int channelNumber)
        {
            ...
            while (true)
            {
                ...
                {
                    var anim = (AmalAnim)instr;
                    var infinite = anim.Times <= 0;
                    var iteration = 0;
                    while (infinite || iteration < anim.Times)
                    {
                        foreach (var frame in anim.Images)
                        {
                            lock (amalLock)
                            {
                                if (!amalInfos.Any(i => i.Number == channelNumber)) return;
                            }

                            // banks.Bank1.Data can be cleared/reloaded (Erase/Load) by the
                            // main thread at any point, concurrently with this background
                            // thread - a plain List<T> isn't safe against that regardless
                            // of the bounds check, so this is deliberately belt-and-suspenders:
                            // bail out quietly (not with an unhandled exception) if the bank
                            // moved out from under us, instead of crashing the whole app.
                            ImageData imageData;
                            try
                            {
                                var imageIndex = frame.Image - 1;
                                if (imageIndex < 0 || imageIndex >= banks.Bank1.Data.Count) return;
                                imageData = banks.Bank1.Data[imageIndex];
                            }
                            ...
                            if (info.Bob != null)
                            {
                                info.Bob.Data = imageData;
                                info.Bob.IsModified = true;
                            }
                            else if (info.Sprite != null)
                            {
                                info.Sprite.Data = imageData;
                                info.Sprite.IsModified = true;
                            }
                            UpdateDisplay(true);

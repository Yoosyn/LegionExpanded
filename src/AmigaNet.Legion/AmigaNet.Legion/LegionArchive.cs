            ...
            do {
                if (screens.MouseClick() == 1)
                {
                    ...
                    if (STREFA > 0 && STREFA < 6)
                    {
                        ...
                        // AMOS: "Change Mouse 42 : Amal Off : Show" - BUSY_ANIM's
                        // Hide On (real cursor hidden while its own spinner sprite
                        // is shown) was never paired with the matching Show here,
                        // so the mouse pointer stayed invisible after every save.
                        screens.ShowOn();

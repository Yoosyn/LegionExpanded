...
namespace AmigaNet.Legion.DesktopApp
{
    public class LegionGame : Game, IGameEngine
    {
        ...
        private Matrix displayScaleMatrix;
        ...
        {
            var w = (float)GraphicsDevice.Viewport.Width;
            var h = (float)GraphicsDevice.Viewport.Height;
            displayScaleMatrix = Matrix.CreateScale(w / WorldWidth, h / WorldHeight, 1f);
        }

        public LegionGame(String langId, String resourcesPath, String dataPath, float scale = 1f)
        {
            baseScale = scale;
            _graphics = new GraphicsDeviceManager(this);
            _graphics.PreferredBackBufferWidth = (int)(WorldWidth * scale);
            _graphics.PreferredBackBufferHeight = (int)(WorldHeight * scale);
            ...
            displayScaleMatrix = Matrix.CreateScale(scale);
            ...
            legion = new Legion(langId, resourcesPath, dataPath, this);
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            mouseX = (int)(mouseState.X * WorldWidth / GraphicsDevice.Viewport.Width);
            mouseY = (int)(mouseState.Y * WorldHeight / GraphicsDevice.Viewport.Height);
            var newMouseKey = 0;
        ...
        private void DrawScreens()
        {
            foreach (var screen in legion.ScreensManager.Screens.ToArray())
            {
                if (screen.IsVisible)
                {
                    ...
                    _spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, matrix * displayScaleMatrix);
                    ...
                    {
                        // AMOS "Priority On" (enabled once in SETUP, effectively for
                        // the rest of the game) draws Bobs ordered by Y so closer/lower
                        // ones occlude farther/higher ones - never implemented in this
                        // port, so overlapping creatures drew in arbitrary (list) order.
                        foreach (var bob in screen.Bobs.ToArray().OrderBy(b => b.Y))
                        {
        ...
        private void DrawSprites()
        {
            for (var i = 0; i < legion.ScreensManager.Display.Sprites.Length; i++)
            {
                ...
                if (sprite != null)
                {
                    ...
                    _spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, scale2Matrix * displayScaleMatrix);
        ...
        public void PlaySample(sbyte[] pcm, int frequencyHz)
        {
            if (pcm == null || pcm.Length == 0) return;

            // MonoGame's SoundEffect requires a sample rate in [8000, 48000];
            // AMOS sample frequencies are occasionally lower, so this clamp
            // trades a slight pitch shift on those for not crashing.
            var sampleRate = Math.Clamp(frequencyHz, 8000, 48000);

            var pcm16 = new byte[pcm.Length * 2];
            for (var i = 0; i < pcm.Length; i++)
            {
                var sample16 = (short)(pcm[i] * 256);
            ...
            Invoke(() =>
            {
                var soundEffect = new SoundEffect(pcm16, sampleRate, AudioChannels.Mono);
                soundEffect.Play();
            });
        }
        ...
        {
            // Used for whole screens and CurrentShape (Bar/Box/Draw/Plot output),
            // never for a pasted object - AMOS's "colour 0 = transparent hole"
            // masking is an object-paste concept (see ImageData.UseMask), not a
            // screen-compositing one. ImageData defaults UseMask to true, which
            // would otherwise make colour-0 areas of a screen/shape punch through
            // to whatever renders behind them (e.g. a stale previous frame) -
            // this is the fix for the shop background / equipment slot
            // "incorrect transparency" issue: screens/shapes must always be
            // fully opaque.
            var imageData = new ImageData("", data, width, height) { UseMask = false };
            return Texture2DFromImageData(imageData);
        }
